...
func (a *App) GetVMCapability() VMCapability {
	...
	if !info.Found {
		return VMCapability{Available: false, Reason: "No installed Linux disk was found."}
	}
...
func (a *App) BootInVM() error {
	...
	args := []string{
		...
		"-m", "4G", "-smp", "4",
		"-machine", "q35",
		"-drive", vmDiskDrive(cap.DiskPath),
...
func (a *App) TryInVMFresh(imageRef string) error {
	...
	f, err := os.OpenFile(previewRaw(), os.O_CREATE|os.O_EXCL|os.O_RDWR, 0o600)
	if err != nil {
		return fmt.Errorf("create VM disk (an existing disk is preserved): %w", err)
	}
	...
	go func() {
		if err := a.runBuilderVM(imageRef, cap); err != nil {
		...
		a.emitVM(VMEvent{Stage: "started",
			Message: "The VM process started. Check its window; Linux desktop readiness is not yet verified."})
	}()
	return nil
}
...
func (a *App) runBuilderVM(imageRef string, cap VMCapability) error {
	...
	go func() {
		...
		defer conn.Close()
		result = readBuilderProgress(conn, a.emitVM)
	}()

	waitErr := cmd.Wait()
	<-done
	if err := result.verify(waitErr); err != nil {
		return err
	}
