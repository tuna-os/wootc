...
Everything above is verified by machines that already know what wootc is. The
trust surface is the opposite: what a stranger's Windows says about our files
*before* anything runs — SmartScreen, the UAC publisher line, the properties
dialog, whether winget has heard of us. No E2E run can observe it, because the
harness never asks Windows what it thinks of the binary.

[#241] verifies it on two machines that have never seen wootc — a clean
Windows 11 VM and a real one:
...
It grades all four criteria from evidence, writes `checklist.md`, and exits
non-zero if any box failed:
...
Three screenshots cannot be automated and must be attached by hand: the UAC
prompt, the exe's **Properties ▸ Details** tab, and the SmartScreen
interstitial (or its absence).

### Two things will fail today, on purpose

Running this now reports ✘, and the checklist is how that stays visible
instead of becoming folklore:

1. **Nothing is signed.** No signing step exists in `release.yml`. Picking and
   buying the signing path is [#229] (a spend decision that needs the
   maintainer); wiring it into the pipeline is [#230]. Until both land, every
   signature box is ✘, SmartScreen shows the unrecognized-app wall, and UAC
   says "unknown publisher".

2. **No build carries a VERSIONINFO resource at all** — so criterion 4's
   fourth item has nothing to check, and the properties dialog it asks for a
   screenshot of is blank. `app/rsrc_windows_amd64.syso` is generated by
   `just build-icon`, which runs `rsrc -ico -manifest`: **`rsrc` emits an icon
   and a manifest and nothing else.** The `info` block in `app/wails.json`
   (`companyName`, `productName`, `productVersion`, `copyright`) describes
   metadata that never reaches a shipped binary, because releases are cut with
   plain `go build` rather than `wails build`.

   Confirm on any build:
   ...
   Note this is also a **branding** gap, not only a signing one: one `.syso` is
   linked into all five builds, and `-ldflags -X main.brandID=…` cannot change
   a resource. Adding version info therefore has to be **per brand**, generated
   at build time, or every branded exe's properties dialog would say `wootc` —
   the exact text criterion 4 forbids. Whoever takes [#230] is already changing
   this build loop and should carry it; `rsrc` cannot do it, so the tool has to
   change too.

   Whatever replaces it **must keep the manifest**: `wootc.manifest` carries
   `requestedExecutionLevel level="requireAdministrator"`, and losing it would
   silently stop the installer elevating. `tests/unit/fresh-machine-trust.bats`
   guards that.
