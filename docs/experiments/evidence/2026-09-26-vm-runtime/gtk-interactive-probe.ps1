Start-Transcript -Path 'C:\Windows\Temp\wootc-runtime-bundle-probe\gtk-interactive-transcript.log' -Force | Out-Null
...
$serial = "$dir\gtk-interactive-serial.log"
...
$p = Start-Process $qemu -WorkingDirectory $dir -ArgumentList $arguments -RedirectStandardOutput "$dir\gtk-interactive-out.log" -RedirectStandardError "$dir\gtk-interactive-err.log" -PassThru
$ready = $false
try {
  ...
  Get-Content "$dir\gtk-interactive-err.log" -ErrorAction SilentlyContinue
