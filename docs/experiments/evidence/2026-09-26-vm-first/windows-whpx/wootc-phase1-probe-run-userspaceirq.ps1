...
foreach ($mem in @(2048,4096)) {
  $prefix = Join-Path $dir "whpx-userspaceirq-$mem"
  $serial = "$prefix-serial.log"
  $timer = [Diagnostics.Stopwatch]::StartNew()
  $args = @('-machine','q35','-accel','whpx,kernel-irqchip=off','-cpu','max','-smp','2','-m',"$mem",'-nodefaults','-no-reboot','-display','none','-monitor','none','-serial',"file:$serial",'-kernel',"$dir\probe-vmlinuz",'-initrd',"$dir\probe-initramfs.cpio.gz",'-append','"console=ttyS0 panic=-1"')
  ...
  $ready = $false
  $peak = 0
  while (-not $p.HasExited -and $timer.Elapsed.TotalSeconds -lt 60) {
    $p.Refresh()
